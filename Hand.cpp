...
Hand::Hand(cv::Mat xyzMap, float angle_threshhold, int cluster_thresh, int id)
{
    this->id = id;
...
void Hand::analyzeHand(cv::Mat xyzMap)
{
    ...
    {
        if (contours_poly[i].size() > contours_poly[maxId].size())
            maxId = i;
    }

    contour = contours_poly[maxId];

    cv::Mat img = cv::Mat::zeros(input.rows, input.cols, CV_8UC3);
    auto color = cv::Scalar(0, 255, 0);

    // generate name for this test case 
    std::stringstream sstm;
    sstm << setfill('0') << setw(6) << id;

    auto testName = sstm.str();

    auto contourPath = "contour\\" + testName + ".txt",
         depthPath = "depth\\" + testName + ".png",
         edgesPath = "edges\\" + testName + ".png";

    ofstream ofsContour(contourPath);

    for (auto pt : contour) {
        ofsContour << pt.x << " " << pt.y << "\n";
    }
    ...
    xyzMap.convertTo(depthMap, CV_16UC1, 1 << 16);
    ...
    while (!stk.empty()) {
        ...
        innerContours.push_back(contours_poly[f]);

        if (f != maxId && hierarchy[f][0] != -1) {
            stk.push_back(hierarchy[f][0]);
        }

        if (hierarchy[f][2] != -1) {
            stk.push_back(hierarchy[f][2]);
        }
    }

    for (auto i = 0; i < innerContours.size(); i++)
    {
        cv::drawContours(img, innerContours, i, color, 1, 8, std::vector<cv::Vec4i>(), 0, cv::Point());
    }

    cv::imwrite(edgesPath, img);
    

    //cv::namedWindow(name, CV_WINDOW_AUTOSIZE);
    //cv::imshow(name, img_dst);

    // don't really care about rest of the stuff
    //return;
