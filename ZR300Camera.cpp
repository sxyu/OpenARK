...
namespace ark {
  ...
  void ZR300Camera::initCamera() {
    ...
    //enable motion module and assign callbacks
  ...
  bool ZR300Camera::nextFrame() {
    update();
    return true;
  }

  void ZR300Camera::update() {
    initializeImages();
    ...
    // populate depthMap;
    std::memcpy((void*) depthMap.datastart, depth_image, REAL_HI * REAL_WID * sizeof(short));
    // populate RGB image
    std::memcpy((void*) rgbMap.datastart, rgb_image, REAL_HI * REAL_WID * 3 * sizeof(unsigned char));
    // populate FishEye image
    std::memcpy((void*) fishEyeMap.datastart, fish_image, REAL_HI * REAL_WID * sizeof(unsigned char));

    // Populate xyzMap
    for (int dy = 0; dy < REAL_HI; ++dy) {
      for (int dx = 0; dx < REAL_WID; ++dx) {
        uint16_t depth_value = depth_image[dy * REAL_WID + dx];
        xyzMap.at<cv::Vec3f>(dy, dx) = deproject(cv::Point2f(dx, dy), depth_intrinsics, depth_value * mDepthScale);
      }
    }
  ...
  void ZR300Camera::fillInZCoords() {
  ...
  void ZR300Camera::fillInRGBImg() {
    uint8_t * color_image = (uint8_t *)mpDev->get_frame_data(rs::stream::rectified_color);
    std::memcpy((void*) rgbMap.datastart, color_image, REAL_HI * REAL_WID * 3 * sizeof(unsigned char));
  }

  void ZR300Camera::fillInFishEyeImg() {
    mTimeStamp = mpDev->get_frame_timestamp(rs::stream::fisheye);
    uint8_t * fisheye_image = (uint8_t *)mpDev->get_frame_data(rs::stream::fisheye);
    std::memcpy((void*) fishEyeMap.datastart, fisheye_image, REAL_HI * REAL_WID * sizeof(unsigned char));
  }

  void ZR300Camera::initializeImages() {
    xyzMap = cv::Mat::zeros(getHeight(), getWidth(), CV_32FC3);
    rgbMap = cv::Mat::zeros(getHeight(), getWidth(), CV_8UC3);
    fishEyeMap = cv::Mat::zeros(getHeight(), getWidth(), CV_8UC1);
    depthMap = cv::Mat::zeros(getHeight(), getWidth(), CV_16SC1);
  }

  void ZR300Camera::destroyInstance() {
    mpDev->stop();
  }

  bool ZR300Camera::hasRGBImage() const {
    return true;
  }

  bool ZR300Camera::hasIRImage() const {
    return false;
  }
  ...
  cv::Mat ZR300Camera::getFishEyeMap() const
  {
    std::lock_guard<std::mutex> lock(imageMutex);
    if (fishEyeMap.data == nullptr) return cv::Mat::zeros(getHeight(), getWidth(), CV_8UC1);
      return fishEyeMap.clone();
  }
