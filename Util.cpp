...
double Util::clusterShortestPath(const cv::Mat & m, cv::Point src, cv::Point sink) {
    static const int INF = 0x3f3f3f3f; // large number representing infinity

    if (!Util::pointInImage(m, src) || !Util::pointInImage(m, sink)) return INF;
    ...
    memset(dist, INF, N * sizeof(double));
    ...
    // allow jump of 5 pixels to help ignore small black gaps
    ...
    while (!st.empty()) {
        ...
        for (int i = 0; i < nNxtPoints; ++i) {
            ...
            const cv::Vec3f & adjXyz = m.at<cv::Vec3f>(pt);
